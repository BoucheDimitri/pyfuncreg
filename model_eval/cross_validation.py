...
class KfoldsCrossVal:
    ...
    def __call__(self, reg, X, Y):
        ...
        for train_index, test_index in inds_split:
            ...
            reg.fit(Xsub_train, Ysub_train, self.input_data_format, self.output_data_format)
            # Predict on validation subset
            preds = reg.predict_evaluate_diff_locs(Xsub_test, Ysub_test[0], input_data_format=self.input_data_format)
