...
class SeperableKPLLearnDict(FunctionalRegressor):
    """
    Parameters
    ----------
    kernel : callable
    ...
    regu_h : float
        Regularization parameter
    regu_dict: float
        Regularization parameters for dictionary
    center_output : bool
        Should output be centered
    """
    def __init__(self, regu_h, regu_dict, kernel, B, nevals_dict, center_output=False):
        super().__init__()
        self.kernel = kernel
        self.regu_h = regu_h
        self.regu_dict = regu_dict
        self.alpha = None
        self.X = None
        self.Ymean_func = None
        # If a basis is given, the output dictionary is fixed, else it is generated from the passed config upon fitting
